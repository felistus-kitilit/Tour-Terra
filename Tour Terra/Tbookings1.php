...
$stmt = $conn->prepare("SELECT * FROM bookings WHERE Tourist = ? ORDER BY BookingID DESC");
$stmt->bind_param("s", $Tourist);
$stmt->execute();
        ...
        <?php foreach ($bookings as $booking): ?>
            ...
            <input type="hidden" id="id" name="id" value="<?php echo htmlspecialchars($booking['BookingID']); ?>" >
                ...
                <?php
                    ...
                    if ($booking['Status'] == 'booked' && $booking['Date'] < $current_date): ?>
                        <div>
                            <a href="rate.php?FarmName=<?php echo $booking['FarmName']; ?>&rating=1&BookingID=<?php echo $booking['BookingID']; ?>">
                                <img src="Images/like.png" alt="Like" style="height: 30px; width: 30px;">
                            </a>
                            <a href="rate.php?FarmName=<?php echo $booking['FarmName']; ?>&rating=0&BookingID=<?php echo $booking['BookingID']; ?>">
                                <img src="Images/dislike.png" alt="Dislike" style="height: 30px; width: 30px;">
                            </a>
                        </div>
                    <?php endif; ?>
                <?php
                $current_date = date("Y-m-d");
                if ($booking['Status'] == 'booked' && $booking['Date'] >= $current_date): ?>
                    <button type="submit" name="action" value="Cancel" style="background-color:maroon;color:white" onclick="return confirm('Are you sure you want to cancel this booking?');">Cancel</button>
                <?php endif; ?>
            </form>
